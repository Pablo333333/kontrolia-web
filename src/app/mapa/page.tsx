...
import { Loader2, Filter, MapPin, FileText, Ticket } from 'lucide-react';
              ...
              <Popup className="min-w-[300px]">
                <div className="space-y-3 p-1">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">{marker.statusName || (marker as any).statusName}</Badge>
                  </div>
                  <h3 className="font-bold text-sm">{(marker as any).title || `${marker.tipo} - ${marker.destinatarioName}`}</h3>
                  <p className="text-xs text-muted-foreground line-clamp-2">{(marker as any).description || 'Sin descripción adicional'}</p>
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                      <DialogHeader>
                        <DialogTitle>Documento Vivo: {(marker as any).title || marker.tipo}</DialogTitle>
                      </DialogHeader>
...
// Helper Button component since we are in a raw file
